...
from data.loader_v2 import AnySpeciesPoseDataset, collate_anyspecies_padded, MAX_JOINTS
...
def run_evaluation(
    ...
    do_vis = is_main_process() and (epoch + 1) % cfg["train"]["vis_every"] == 0
    if do_vis:
        ...
        for bi, batch in enumerate(tqdm_loader):
            ...
            if pred.size(1) > 1:
                ...
                speed_l1 = masked_loss(
                    pred_diff, target_diff, joint_mask_for_loss_t, nn.L1Loss(reduction="none")
                )
                speed_l2 = masked_loss(
                    pred_diff, target_diff, joint_mask_for_loss_t, nn.MSELoss(reduction="none")
                )
...
def build_test_dataloaders(data_cfg, eval_cfg, seq_len, distributed, rank, world_size):
...
def train_video2pose_v3(cfg):
    ...
    logdir = os.path.join(base_dir, "logs_video2pose_v3")
    if is_main_process():
    ...
    vel_loss_type = train_cfg.get("vel_loss_type", train_cfg["loss_type"])
    vel_criterion = get_loss_fn(vel_loss_type)
    vel_weight = float(train_cfg.get("vel_weight", 0.0))
    ...
    for epoch in range(start_epoch, epochs):
        if distributed and sampler_train is not None:
        ...
        for i, batch in loader_tqdm:
            ...
            with torch.cuda.amp.autocast():
                pred = model(batch)
                loss_pose = masked_loss(pred, target, joint_mask_for_loss, criterion)
                raw_loss = loss_pose
                if vel_weight > 0:
                    loss_vel = pos_vel_loss(pred, target, joint_mask_for_loss, vel_criterion)
                    raw_loss = raw_loss + vel_weight * loss_vel
            ...
            if writer is not None and global_step % 10 == 0:
                ...
                writer.add_scalar("train/loss_pose", loss_pose.item(), global_step)
        ...
        if is_main_process():
            logger.info(
                f"Epoch {epoch + 1}: train loss={train_loss:.6f} | time {epoch_time:.1f}s"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Training script for Video2Pose v3")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/train_video2pose_v3.yaml",
        help="Path to the YAML config file",
    )
    args = parser.parse_args()

    cfg = load_yaml_config(args.config)
    train_video2pose_v3(cfg)
