...
class Quaternions:
    ...
    def __truediv__(self, other):
        ...
        if isinstance(other, float):
            return self * (1.0 / other)
        raise TypeError(
            'Cannot divide/subtract Quaternions with type %s' +
            str(type(other))
        )
    ...
    def log(self):
        ...
        imgs = imgs * lens[..., np.newaxis]
        return imgs

        # theta = np.arccos(self.reals)
        # while len(np.where(theta > np.pi)[0]) > 0:
        #     theta = np.where(theta > np.pi, theta - 2*np.pi, theta)
        # imgs1 = self.imaginaries

        # sins = np.sin(theta)
        # sins[sins == 0] = 0.00001
        # # print(imgs.shape, theta.shape, sins.shape)
        # imgs1 = theta[...,np.newaxis] * imgs1 / sins[...,np.newaxis]

        # # print(np.sum(imgs1 - imgs))
    ...
    def average(self):
        ...
        if len(self.shape) == 1:
            ...
            return Quaternions(
                v[:, np.argmin((1. - qiT_dot_qref**2).sum(axis=0))]
            )
    ...
    def between(cls, v0s, v1s):
        a = np.cross(v0s, v1s)
        w = np.sqrt((v0s**2).sum(axis=-1) *
                    (v1s**2).sum(axis=-1)) + (v0s * v1s).sum(axis=-1)
        return Quaternions(np.concatenate([w[..., np.newaxis], a],
                                          axis=-1)).normalized()

    @classmethod
    def from_angle_axis(cls, angles, axis):
        axis = (
            axis / (np.sqrt(np.sum(axis**2, axis=-1)) + 1e-10)[..., np.newaxis]
        )
    ...
    def from_transforms(cls, ts):
        ...
        # print(ts.shape, c0.shape, ts[c0, 2, 1].shape, q1.shape, q1[c0].shape)
        # print(c0, np.sum(c0))
        # print(q1)
        # q1[c0] += 100
        # print(q1[c0])
    ...
    @classmethod
    def set_values(cls, new_qs):
        return Quaternions(new_qs)
